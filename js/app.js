(() => {
    ...
    (() => {
        ...
        function menuInit() {
            if (document.querySelector(".icon-menu")) document.addEventListener("click", (function(e) {
                if (bodyLockStatus && e.target.closest(".icon-menu")) {
                    ...
                    document.querySelector(".open-submenu") ? document.querySelector(".open-submenu").classList.remove("open-submenu") : null;
        ...
        function initSliders() {
            ...
            if (document.querySelector(".mission__body")) new Swiper(".mission__body", {
                ...
                navigation: {
                    prevEl: ".mission-item-prev",
                    nextEl: ".mission-item-next"
                },
                breakpoints: {
                    320: {
                        autoHeight: true
            ...
            if (document.querySelector(".mission-next__body")) new Swiper(".mission-next__body", {
                modules: [ Navigation, Pagination ],
                observer: true,
                observeParents: true,
                slidesPerView: 1,
                ...
                navigation: {
                    prevEl: ".mission-item-02-prev",
                    nextEl: ".mission-item-02-next"
                },
                breakpoints: {
                    320: {
                        autoHeight: true
            ...
            if (document.querySelector(".benefits__body")) resizableSwiper("(max-width: 479.98px)", ".benefits__body", {
                modules: [ Pagination ],
                observer: true,
                observeParents: true,
                slidesPerView: 1,
            ...
            if (document.querySelector(".blog-main__body")) resizableSwiper("(max-width: 479.98px)", ".blog-main__body", {
                modules: [ Pagination ],
                observer: true,
                observeParents: true,
                slidesPerView: 1,
        ...
        function documentActions(e) {
            ...
            const targetElement = e.target;
            if (targetElement.closest(".header__search-toggle")) if (!searchForm.classList.contains("active")) {
            ...
            if (window.matchMedia("(max-width: 1299.98px)").matches) if (!searchForm.contains(targetElement) && !searchToggle.contains(targetElement)) {
                searchForm.classList.remove("active");
                setTimeout((function() {
                    searchForm.style.display = "none";
                }), 300);
            }
            if (targetElement.closest(".menu__item.has-submenu > a")) {
                const submenu = targetElement.nextElementSibling, activeLink = document.querySelector(".active-link"), activeMenu = document.querySelector(".open-submenu");
            ...
            menuItems.forEach((item => {
                ...
                item.addEventListener("mouseleave", (function(e) {
                    document.addEventListener("mousemove", (function(e) {
                        const isOutsideMenu = !item.contains(e.target) && !submenu.contains(e.target);
                        if (isOutsideMenu) {
        ...
        if (document.querySelector(".text-section__item")) {
            const targetBlocks = document.querySelectorAll(".text-section__item");
            const scrollContainer = document.querySelector("#scroll-container");
            let lastVisibleIndex = -1;
            let scrollContainerFullyVisible = false;
            const updateClasses = scrollPos => {
                const scrollThreshold = 150;
                const currentIndex = Math.floor(scrollPos / scrollThreshold) - 12;
                if (currentIndex !== lastVisibleIndex) if (currentIndex >= 0 && currentIndex < targetBlocks.length) {
                    if (lastVisibleIndex >= 0 && lastVisibleIndex < targetBlocks.length) targetBlocks[lastVisibleIndex].classList.remove("show");
                    targetBlocks[currentIndex].classList.add("show");
                    lastVisibleIndex = currentIndex;
                }
            };
            const checkScrollContainerVisibility = entries => {
                entries.forEach((entry => {
                    if (entry.isIntersecting && entry.intersectionRatio === 1) {
                        scrollContainerFullyVisible = true;
                        if (targetBlocks.length > 0) {
                            targetBlocks[0].classList.add("show");
                            lastVisibleIndex = 0;
                        }
                    } else {
                        scrollContainerFullyVisible = false;
                        targetBlocks.forEach((block => block.classList.remove("show")));
                        lastVisibleIndex = -1;
                    }
                }));
            };
            const containerObserver = new IntersectionObserver(checkScrollContainerVisibility, {
                rootMargin: "0px",
                threshold: 1
            });
            containerObserver.observe(scrollContainer);
            const handleScroll = () => {
                if (scrollContainerFullyVisible) {
                    const scrollPos = window.scrollY - scrollContainer.offsetTop;
                    if (scrollPos >= 0) updateClasses(scrollPos);
                }
            };
            window.addEventListener("scroll", handleScroll);
        }
        let myElement = document.querySelector(".widget-up");
        let lastScrollTop = 0;
